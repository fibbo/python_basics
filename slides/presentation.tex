...
\subsection{Variables, Types, Expressions, Operators, Comments} % (fold)
...
\begin{frame}[c, fragile, allowframebreaks]\frametitle{Data Types}
	\begin{block}{Strings}
		\begin{itemize}
			\item Strings (Text)
			      \begin{itemize}
	...
	\begin{block}{None Type}
		\begin{itemize}
			\item \texttt{None} represents "nothing" or "no value"
			\item Think of it like an empty box - the box exists, but there's nothing inside
			\item Common examples:
			      \begin{itemize}
				      \item A person's middle name when they don't have one
				      \item The result when a function doesn't return anything
				      \item An uninitialized variable
			      \end{itemize}
		\end{itemize}
	\end{block}
	\begin{examples}
	...
	\begin{columns}
		...
		\begin{column}{0.2\textwidth}
			...
			\begin{tikzpicture}[scale=0.8]
				\draw[fill=red, draw=red, rounded corners=3pt]
				(0,0) -- (1.5,0) -- (0.75,1.3) -- cycle;
				\node[white, font=\Large\bfseries] at (0.75,0.4) {!};
			\end{tikzpicture}
		\end{column}
	\end{columns}
...
\subsection{Input and Output} % (fold)
...
\usebeamercolor[fg]{normal text}
\bfseries
\begin{frame}[c]\frametitle{Exercise 2}
	Solve exercise 2
\end{frame}
}
...
\end{lstlisting}
	...
	\bblock{Length}{
		\begin{itemize}
			\item The number of elements in a list can be obtained using the \texttt{len()} function
		\end{itemize}
	}
...
\end{lstlisting}
	...
	{
		\begin{itemize}
			\item The \texttt{append()} method can be used to add an element at the end of the list
		\end{itemize}
	}
...
\subsubsection{User-Defined Functions (without and with return)} % (fold)
...
\usebeamercolor[fg]{normal text}
\bfseries
\begin{frame}[c]\frametitle{Exercise 6}
	Solve exercise 6
\end{frame}
}
...
\begin{frame}[c, allowframebreaks, fragile]\frametitle{Dictionaries}
	...
	\bblock{Key-Value pair}{
		\begin{itemize}
			\item Dictionaries are similar to lists but have a key and value for each entry
		\end{itemize}
	}
...
\end{lstlisting}
	...
	\bblock{Deleting entries}{
		\begin{itemize}
			\item Key-Value pairs can be deleted using the \texttt{del()} function
		\end{itemize}
	}
...
\end{lstlisting}
	...
	\bblock{Why pathlib?}{
		\begin{itemize}
			\item Modern, object-oriented approach to file handling (Python 3.4+)
			\item Cross-platform path handling (works on Windows, Mac, Linux)
			\item More intuitive methods for common file operations
			\item Better path manipulation with / operator
		\end{itemize}
	}
...
\end{lstlisting}
	...
	\bblock{Write}{
		\begin{itemize}
			\item The write\_text() method is a convenient way to write strings
			\item The open() method with "a" mode can be used for appending
...
\end{lstlisting}
	...
	\bblock{Read}{
		\begin{itemize}
			\item Path.read\_text() reads the entire file as a string
...
\begin{frame}[c, fragile, allowframebreaks]\frametitle{JSON}
	...
	\bblock{Dictionaries/list in JSON}{
		\begin{itemize}
			...
			\item If complex structures such as dictionaries or lists should be stored in a file, it's necessary to convert these structures into strings first
...
\end{lstlisting}
	...
	\bblock{Combining pathlib with JSON}{
		\begin{itemize}
			\item Path makes it easy to save and load JSON files
			\item write\_text() and read\_text() work perfectly with JSON
		\end{itemize}
	}
...
file_path.write_text(json.dumps(my_dict, indent=2))

# Load dictionary from JSON file
loaded_dict = json.loads(file_path.read_text())
print(loaded_dict)
\end{lstlisting}
